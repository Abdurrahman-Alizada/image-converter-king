...
const ConvertToJPGIndex = ({route, navigation}) => {
  ...
  const convertAndSaveToJPG = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedImages.length; i++) {
        ...
        let fileName = `${selectedImages[i].fileName.split('.')[0]}.jpg`;
        let filePath = `${folderPath}/${fileName}`;
        let fileExists = await RNFS.exists(filePath);

        let count = 1;
        while (fileExists) {
          fileName = `${selectedImages[i].fileName.split('.')[0]} (${count}).jpg`;
