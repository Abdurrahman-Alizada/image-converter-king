...
const ConvertToSVGIndex = ({route, navigation}) => {
  ...
  const convertAndSaveToSVG = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedImages.length; i++) {
        ...
        let fileName = `${selectedImages[i].fileName.split('.')[0]}.svg`;
        let filePath = `${folderPath}/${fileName}`;
        let fileExists = await RNFS.exists(filePath);

        let count = 1;
        while (fileExists) {
          fileName = `${selectedImages[i].fileName.split('.')[0]} (${count}).svg`;
  ...
  return (
    <View style={{flexGrow: 1, backgroundColor: theme.colors.background}}>
      <ConvertToOtherFormatAppbar title={`Convert to SVG`} />
        ...
        <View style={{alignItems: 'center'}}>
          {convertLoading ? (
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              ...
              <Text>Converting image to SVG</Text>
            </View>
          ) : (
            <Button
              onPress={convertAndSaveToSVG}
              ...
              {`Convert ${selectedImages?.length} images to SVG`}
            </Button>
          )}
        </View>
      )}
...
export default ConvertToSVGIndex;
